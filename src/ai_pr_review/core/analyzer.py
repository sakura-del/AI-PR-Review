...
from ai_pr_review.core.config import AppConfig, load_project_config
from ai_pr_review.core.context_builder import ContextBuilder
from ai_pr_review.core.expert_knowledge import select_experts, get_expert_profiles, merge_expert_config
from ai_pr_review.core.prompt_templates import build_analysis_prompt
from ai_pr_review.data.team_rules import load_team_pattern, merge_team_rules
...
class AIAnalyzer:
    ...
    def _build_ctx(
        ...
        """从 ContextBuilder 输出构造 AnalysisContext

        把 4 个 analyze* 方法共用的"从 context dict 提取字段 + 注入项目配置"逻辑收敛到一处。
        v0.9 引入 AnalysisContext 后所有 prompt 构造都走这里。
        """
    ...
    async def analyze(
        ...
        messages = build_analysis_prompt(self._build_ctx(context, experts))
    ...
    async def analyze_multi_agent(
        ...
        """
        from ai_pr_review.core.multi_agent import run_multi_agent_review
        from ai_pr_review.core.aggregator import aggregate_results
        from ai_pr_review.core.adversarial import adversarial_filter
    ...
    async def analyze_incremental(
        ...
        messages = build_analysis_prompt(
            self._build_ctx(context, experts, incremental_context=incremental_context)
        )
        ...
        try:
            for attempt in range(AI_MAX_RETRIES):
                try:
                    ...
                    # 提取 token 用量（如响应携带 usage 字段）
                    _record_token_usage(response, self._config.ai.model)
                    return response.choices[0].message.content or ""
                except Exception as e:
                    last_error = e
                    # 异常路径：记录降级失败（每次重试失败都计入；token 超限等也会计入）
...
def _record_token_usage(response, model: str) -> None:
    """从 AI 响应中提取 token 用量，写入 llm_token_usage_total{model, type} 指标

    兼容 OpenAI / DeepSeek / Qwen / GLM 的 usage 字段：
        response.usage.prompt_tokens
        response.usage.completion_tokens

    若响应不携带 usage（部分 provider 流式响应），静默跳过。
    """
    ...
    counter = get_registry().counter(
        ...
        description="LLM token 用量累计（按 model + type 维度）",
        labels=("model", "type"),
    )
    if prompt_tokens is not None:
        try:
            counter.inc(value=float(prompt_tokens), model=model, type="prompt")
        except (TypeError, ValueError):
            pass
    if completion_tokens is not None:
        try:
            counter.inc(value=float(completion_tokens), model=model, type="completion")
        except (TypeError, ValueError):
            pass
